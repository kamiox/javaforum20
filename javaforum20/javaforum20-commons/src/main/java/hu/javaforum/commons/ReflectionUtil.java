...
import hu.javaforum.util.internal.ReflectionHelper;
...
/**
 * The ReflectionUtil is provides static methods to invoke getter and setter
 * methods in JavaBean instances. It is based on the ReflectionHelper class.
 *
 * Changelog:
...
public final class ReflectionUtil extends ReflectionHelper
{
   ...
   */
  private static final Logger LOGGER = new Logger(ReflectionUtil.class);
  /**
   * The static trace logger instance.
   */
  private static final Tracer TRACER = new Tracer(ReflectionUtil.class);
   ...
   * @return The value; null, if the field is not found
   */
  private static Object invokeGetter(final Class instanceClass,
   ...
   */
  public static Boolean invokeSetter(final Object instance,
   ...
   * @return true, if the invoke done successfully
   */
  private static Boolean invokeSetter(final Class instanceClass, final Object instance,
          final String fieldName, final Object value)
  {
    PerfTracer tracer = new PerfTracer(TRACER);
    PerfLogger logger = new PerfLogger(LOGGER);
    ...
    try
    {
      ...
      /**
       * If the field's class and the value's class is not equals, then
       * need to cast or convert the value to the field's class.
       */
      ...
      if (parameter == null)
      {
        logger.warn("The '%1$s' type isn't supported yet (fieldName was '%2$s')",
    ...
    {
      logger.debug("Invoking %1$s.%2$s(Object object) because %3$s",
              instanceClass.getSuperclass().getName(), fieldName, except.getMessage());
  ...
  /**
   * Checks the encapsulated fields (dot separated field name), and calls
   * recursively the invokeSetter in the parent's field.
   *
   * @param instanceClass The class of instance
   * @param instance The instance
   * @param value The field value
   * @param fieldName The name of the field
   * @return True, if the setter was called successfully; False, if the setter
   * wasn't ran successfully or not found; null, if the field name is not
   * encapsulated.
   */
  private static Boolean decapsulate(final Class instanceClass,
          final Object instance, final Object value, final String fieldName)
  {
    PerfLogger logger = new PerfLogger(LOGGER);
    ...
    {
      logger.debug("Invoking %1$s.%2$s(Object object)", parentFieldName, lastFieldName);
